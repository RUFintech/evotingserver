...
function generateChannelArtifacts() {
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile ComposerOrdererGenesis -outputCreateChannelTx ./channel-artifacts/channel.tx -channelID $CHANNEL_NAME
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile ComposerOrdererGenesis -outputAnchorPeersUpdate ./channel-artifacts/Org1MSPanchors.tx -channelID $CHANNEL_NAME -asOrg Org1
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  # echo "#################################################################"
  # set -x
  # configtxgen -profile ComposerOrdererGenesis -outputAnchorPeersUpdate \
